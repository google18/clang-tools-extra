...
void WrapUniqueCheck::registerMatchers(MatchFinder *Finder) {
  ...
  cxxMemberCallExpr(
    ...
    hasArgument(0, callExpr().bind("callExpr"))).bind("facCons"), this);

  Finder->addMatcher(
  cxxConstructExpr(
      anyOf(hasParent(decl().bind("cons_decl")), anything()),
      hasType(cxxRecordDecl(hasName("std::unique_ptr"))),
      has(callExpr().bind("FC_call"))).bind("upfc"),this);
}
...
void WrapUniqueCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  if(facExpr){
    ...
    newText = ObjName.str() + " = absl::WrapUnique(" + getArgs(SM, callExpr) + ")"; 
  ...
  if(cons){
    ...
    llvm::StringRef NameRef = Lexer::getSourceText(CharSourceRange::getCharRange(
    cons->getBeginLoc(), cons->getParenOrBraceRange().getBegin()),*SM,
    LangOptions());
